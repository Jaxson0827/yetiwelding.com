            ...
            >
              ...
              <div className="relative z-10 p-8 md:p-12 lg:p-16">
                ...
                >
                  ...
                  <p className="text-white/80 text-lg md:text-xl leading-relaxed max-w-3xl mb-4">
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {service.features.map((feature, index) => (
                      <motion.div
                        key={index}
                        className="flex items-start p-4 bg-white/5 backdrop-blur-sm rounded border border-white/10"
                      ...
                      >
                        <span className="w-2 h-2 bg-accent-red rounded-full mr-4 mt-2 flex-shrink-0" />
                ...
                {/* Related Services */}
                {getRelatedServices(service.id).length > 0 && (
                  <motion.div
                    className="mb-12"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                  ...
                  >
                    <h3 className="text-white uppercase text-sm tracking-[0.2em] font-light mb-4">
                      YOU MIGHT ALSO LIKE
                    </h3>
                    <div className="flex flex-wrap gap-3">
                      {getRelatedServices(service.id).map((relatedService) => (
                        <Link
                          key={relatedService.id}
                          href={relatedService.href}
                          onClick={handleClose}
                          className="px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/20 hover:border-accent-red/50 rounded text-white/80 hover:text-white text-sm uppercase tracking-wide transition-all"
                        >
                          {relatedService.name}
                        </Link>
                      ))}
