...
export default function Header({ showCart = true }: HeaderProps) {
  ...
  useEffect(() => {
    ...
    const readCartCount = () => {
      try {
        const stored = window.localStorage.getItem('yeti-welding-cart');
        if (!stored) {
        ...
        setFallbackItemCount(Array.isArray(items) ? items.length : 0);
      } catch {
        setFallbackItemCount(0);
      }
    };
    ...
    return () => window.removeEventListener('storage', readCartCount);
  }, [cart]);
